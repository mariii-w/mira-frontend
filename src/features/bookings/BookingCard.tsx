...
export type LocationType = "AT_CONSUMER" | "AT_PROVIDER" | "REMOTE";
...
export function BookingCard({
  ...
  const [detail, setDetail] = useState<BookingDetails | null>(
    mockDetail ?? null,
  );
  ...
  async function handleToggle() {
    ...
    if (next && !detail && !mockDetail) {
      ...
      try {
        setDetail(await loadBookingDetails(booking.bookingId));
      } finally {
  ...
  async function handleAction(action: AllowedAction) {
    ...
    setActionError(null);
    try {
      const error = await performBookingAction(booking.bookingId, action);
      if (error) {
    ...
    >
      <div className="flex items-start gap-4 p-4 sm:p-5">
        <div className="flex flex-col items-center justify-center min-w-[3rem] text-center select-none self-center">
          ...
          <span
            aria-hidden="true"
            className="text-h2 font-bold text-foreground leading-tight"
          >
            {day}
          </span>
          <span
            aria-hidden="true"
            className="text-xs text-muted leading-none mt-0.5"
          >
            {time}
          </span>
        </div>
      ...
      {expanded && (
        <div
          id={detailsId}
          className="bg-cream border-t border-border/20 px-4 pb-4 pt-3 sm:px-5"
        >
          {loadingDetail && (
            <p
              role="status"
          ...
          {detail && (
            <>
              ...
              {booking.status === "AWAITING_CONFIRMATION" &&
                (detail.allowedActions.some(
                  (a) => a.rel === "acknowledge-delivery",
                ) ? (
                  <p className="text-xs text-muted mb-3">
                    Provider has marked this as done.
                  </p>
                ) : (
                  <p className="text-xs text-muted mb-3">
              ...
              {detail.autoConfirmAt &&
                booking.status === "AWAITING_CONFIRMATION" && (
                  <p className="text-xs text-amber-700 mb-3">
                    ⚠ Auto-confirm if you don't respond
                  </p>
                )}
              {actionError && (
                <p role="alert" className="text-small text-red-600 mb-3">
                  {actionError}
                </p>
              )}
