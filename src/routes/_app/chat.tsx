...
export const Route = createFileRoute("/_app/chat")({
  head: () =>
    createPageMeta({
      title: "Messages",
      description:
        "Read and reply to Mira conversations about services and bookings.",
      path: "/chat",
    }),
...
function Chat() {
  ...
  const [subscribeErrors, setSubscribeErrors] = useState<Set<string>>(
    new Set(),
  );
  ...
  } = useHistory(activeChatId, undefined, {
  ...
  const historyRawIds = new Set(
    historyResponse?.status === 200
      ? historyResponse.data.items.map((m) => m.id)
      : [],
  );
  // Drop live messages already covered by history (avoids dupes on refetch).
  const liveMessages = (messagesByChat[activeChatId] ?? []).filter(
    (m) => !historyRawIds.has(m.rawId),
  );
  ...
  const { data: listingResponse, isError: listingIsError } =
    useGetPublicListing(selectedChat?.listingId ?? "", {
    ...
    ids.forEach((cid) => {
      subscribeToChat(cid, (incoming) => {
        const newMessage: LiveMessage = {
          ...
          timestamp: new Date(incoming.createdAt).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          }),
        };
