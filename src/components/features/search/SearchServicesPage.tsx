...
function toListingsParams(
  params: BrowseServicesSearch,
): GetPublicListingsParams {
...
async function fetchPublicListings(
  params: BrowseServicesSearch,
): Promise<PublicListingCollectionResponse> {
...
export function SearchServicesPage() {
  ...
  const activeCount =
    (search.tagIds.length > 0 ? 1 : 0) + (hasPriceFilter ? 1 : 0);
  ...
  const breadcrumbLinks = [
    ...
    ...(search.q
      ? [
          {
            name: search.q,
            href: `/browse-services?q=${encodeURIComponent(search.q)}`,
          },
        ]
      : []),
  ];
  ...
  if (search.city && search.radiusKm)
    subtitleParts.push(`Within ${search.radiusKm} km`);
            ...
            {hasActiveFilters && (
              ...
              >
                ...
                {activeTagChips.map((tag) => (
                  <span
                    key={tag.tagId}
                    className="inline-flex items-center gap-1 rounded-full border border-border bg-surface px-3 py-1 text-small"
                  >
                    {tag.name}
                    <button
                      type="button"
                      ...
                      onClick={() => {
                        const updatedTagIds = search.tagIds.filter(
                          (id) => id !== tag.tagId,
                        );
        ...
        <div className="mx-auto max-w-6xl pt-1 pb-6 lg:py-6 flex gap-20 items-start">
          ...
          <aside className="hidden lg:block shrink-0 w-64">
            <FilterBar
              ...
              resultCount={
                listingsQuery.isSuccess ? listings.length : undefined
              }
              ...
              <div>
                <div>
                  <h1
                    ...
                    className="font-heading text-h1 font-bold text-foreground"
                  >
                      ...
                      >
                        <ServiceCard
                          ...
                          pictureAltTextStatus={
                            listing.primaryMedia?.altTextStatus
                          }
