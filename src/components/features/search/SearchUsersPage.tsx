...
import type {
  GetPublicProfilesCollectionParams,
  PublicListingSummary,
  PublicProfileCollectionResponse,
} from "../../../api/model";
...
function toProfilesParams(
  params: BrowseUsersSearch,
): GetPublicProfilesCollectionParams {
...
async function fetchPublicProfiles(
  params: BrowseUsersSearch,
): Promise<PublicProfileCollectionResponse> {
...
async function fetchAllProviderListings(
  userId: string,
  ...
  for (;;) {
    ...
    if (response.status !== 200)
      throw new Error("Provider services could not be loaded.");
...
export function SearchUsersPage() {
  ...
  const consumers = profiles.filter(
    (profile) => profile.userType !== "PROVIDER",
  );
  ...
  const cardsLoading =
    profilesQuery.isLoading || (profilesQuery.isSuccess && enrichmentPending);
  ...
  providers.forEach((provider, index) => {
    ...
    if (listings)
      providerSummaries.set(
        provider.userId,
        summarizeProviderServices(listings),
      );
  });
  ...
  function commitSearch() {
    setPrevCursors([]);
    navigate({
      search: { q: pendingQuery, role: search.role, from: undefined },
    });
  }
  ...
  const breadcrumbLinks = [
    ...
    ...(search.q
      ? [
          {
            name: search.q,
            href: `/browse-users?q=${encodeURIComponent(search.q)}`,
          },
        ]
      : []),
  ];
          ...
          >
            ...
            {resultsReady && visibleProfiles.length > 0 && (
              <ul
                ...
                className="grid grid-cols-1 lg:grid-cols-2 gap-4 list-none m-0 p-0"
              >
                {visibleProfiles.map((profile, index) => (
                  ...
                  >
                    <UserCard
                      profile={profile}
                      easyRead={easyRead}
                      providerSummary={providerSummaries.get(profile.userId)}
                    />
                  </li>
                ))}
              </ul>
